...
2. [Broken Access Control](#2-broken-access-control)
   ...
   3. [Product Tampering](#23-product-tampering)
...
4. [Miscellaneous](#4-miscellaneous)
   ...
   2. [CAPTCHA Bypass](#42-captcha-bypass)
5. [Unvalidated Redirects](#5-unvalidated-redirects)
   1. [Outdated Allowlist](#51-outdated-allowlist)
   2. [Allowlist Bypass](#52-allowlist-bypass)
6. [XSS](#6-xss)
   1. [Reflected XSS](#61-reflected-xss)
   2. [Client-side XSS Protection](#62-client-side-xss-protection)
...
As per the hints, I started by looking at some broken admin functionality. However, after logging in as Admin, I only found ways to delete customer feedback in the subdirectory `/#/administration`. Selecting the "OWASP SSL Advanced Forensic Tool (O-Saft)" didn't reveal any way of modifying the product, not even as an admin.

So I went for the second approach, namely digging into the API responsible for handling the products. I remembered from some earlier challenge that there is a public endpoint called `/api/products`. Within this endpoint, I searched for the O-Saft and found it was saved as `"id": 9`:
...
```json
{
      ...
      "createdAt": "2026-02-04T09:55:29.468Z",
      "updatedAt": "2026-02-04T09:55:29.468Z",
      "deletedAt": null
    }
```

Now the goal was to forge a suitable HTTP request to update the `description` field of this product. First, I intercepted a call to `/api/products/9` and sent it to the Burp Suite repeater for further modification. To check which methods could be used, I tried `OPTIONS`:

<div class="flex justify-center items-center">
  <img
    src="https://github.com/user-attachments/assets/c1135a22-d6d7-4e52-945a-767f3a6aaebc"
    alt="image"
    class="clickable-img h-auto w-auto max-w-full max-h-[232px] object-contain"
  />
</div>
...
Next, I added `Content-Type: application/json`, because I knew I'd be sending json data to the endpoint, and I started playing around with POST requests, i.e. trying to send the following content to the server:

```json
{"description":"HACKED! \u003Ca href=\"https://owasp.slack.com\" target=\"_blank\"\u003EMore...\u003C/a\u003E"}
```

I kept getting 500 internal server errors, so I decided to try PUT requests. This turned out to be the solution, successfully modifying the description:

<div class="flex justify-center items-center">
  <img
    src="https://github.com/user-attachments/assets/6c57bc5d-30a6-4482-93ea-281f7cdc9eb5"
    alt="image"
    class="clickable-img h-auto w-auto max-w-full max-h-[232px] object-contain"
  />
</div>
...
<div class="flex justify-center items-center">
  <center>
    <img
      src="https://github.com/user-attachments/assets/26c97a32-292a-4afc-a0b4-f47498d2cbc2"
      alt="image"
      class="clickable-img h-auto w-auto max-w-full max-h-[232px] object-contain"
    />
  </center>
...
<div class="flex justify-center items-center">
  <img
    src="https://github.com/user-attachments/assets/a491151f-30c8-48e1-9daa-f86e0bb58640"
    alt="image"
    class="clickable-img h-auto w-auto max-w-full max-h-[232px] object-contain"
  />
</div>
...
### 5.2 Allowlist Bypass

> Enforce a redirect to a page you are not supposed to redirect to.

Even though this is a 4-star challenge, the solution is rather short. First, I searched through the client-facing javascript file `main.js` for anything related to `redirect?to=`. I got several hits in the following highlighted line (the redirects are out of frame to the right):
<div class="flex justify-center items-center">
  <img
    src="https://github.com/user-attachments/assets/296603bd-d256-4595-b576-8c541817ae3b"
    alt="image"
    class="clickable-img h-auto w-auto max-w-full max-h-[232px] object-contain"
  />
</div>
...
The actual redirects were uninteresting, however the words "app-payment" and "coupon" told me that I should look more at the checkout mechanisms within the Juice Shop. Selcting "Other payment options" shows some links to merchandise. Clicking these will redirect to the respective pages:
<div class="flex justify-center items-center">
  <img
    src="https://github.com/user-attachments/assets/8811b6dc-7c4d-4590-ba5a-01fd7bfe8dc7"
    alt="image"
    class="clickable-img h-auto w-auto max-w-full max-h-[232px] object-contain"
  />
</div>
...
Now I simply tried smuggling in a new URL into the existing one, hoping that there was no mechanism properly checking the input:
<div class="flex justify-center items-center">
  <img
    src="https://github.com/user-attachments/assets/2fadbc1c-b1e8-4424-b1fe-26e1b6e32338"
    alt="image"
    class="clickable-img h-auto w-auto max-w-full max-h-[232px] object-contain"
  />
</div>
...
Adding my homepage and sending the request worked right away and solved the challenge!
...
### 6.2 Client-side XSS Protection

> Perform a persisted XSS attack with `<iframe src="javascript:alert('xss')">` bypassing a client-side security mechanism.

I was a bit clueless which endpoint or input field to check for vulnerabilities at first. One additional hint told me that I should check the mechanism for signing up new users, i.e. I should check the endpoint `/api/users`. Thus, I intercepted a GET request to this endpoint and started customising it in order to forge a malicious new user.

First, I added the header `Content-Type: application/json` since we're talking to an API. Then, I changed the method to POST and added a new user with email being the payload `<iframe src=\"javascript:alert('xss')\">` and some random password (in a possible next step I would've added the payload as the password instead of email). I sent the request and the server added the new user:

<div class="flex justify-center items-center">
  <img
    src="https://github.com/user-attachments/assets/8b713daa-8b1c-46d5-892e-158956489fb2"
    alt="image"
    class="clickable-img h-auto w-auto max-w-full max-h-[232px] object-contain"
  />
</div>
...
In fact, this already triggered the success banner, but I wanted to see the XSS-alert in action. The alert would only possibly be triggered on pages where the malicious email is displayed in plain text. One such area is the admin section at `/#/administration`. Indeed, when I logged in as admin and navigated to this subdirectory the alert was raised:

<div class="flex justify-center items-center">
  <img
    src="https://github.com/user-attachments/assets/8bb5e31e-3ffa-42d2-99da-275e2a8716a3"
    alt="image"
    class="clickable-img h-auto w-auto max-w-full max-h-[232px] object-contain"
  />
</div>
...
<div class="flex justify-center items-center">
  <img
    src="https://github.com/user-attachments/assets/627bc737-0a72-42d6-9798-2fac8151e4eb"
    alt="image"
    class="clickable-img h-auto w-auto max-w-full max-h-[232px] object-contain"
  />
</div>
